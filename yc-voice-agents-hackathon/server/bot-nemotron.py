...
"""Incident-triage voice agent on Nemotron (NVIDIA OSS model).

Forked from the flower-shop starter. Keeps the Nemotron STT
(NVidiaWebSocketSTTService) + Nemotron-3-Super LLM (VLLMOpenAILLMService) +
Gradium TTS + transport plumbing; swaps the flower tools/prompt/greeting for the
shared triage module (see triage.py). An engineer connects to on-call triage;
the bot investigates the active incident with tools, synthesizes the root cause
aloud, can page the right on-shift engineer, and — only after a verbal yes —
applies a scoped code fix.

Pipeline: Nemotron Speech Streaming STT → Nemotron-3-Super LLM → Gradium TTS,
with the shared direct-function triage tools registered on the LLM context.
...
async def run_bot(
    ...
    # Shared triage wiring is set up after the LLM is constructed below
    # (build_triage registers the tools on the llm and gives us the
    # ToolsSchema, event bus, and greeting). The triage system instruction
    # is a module constant passed into the LLM at construction.
    ...
    # llm), greeting. Same module both bots use, so triage logic lives once.
    ...
    async def on_client_connected(transport, client):
        ...
        bus.emit("session_start", {"session_id": session_id, "backend": "nemotron"})
